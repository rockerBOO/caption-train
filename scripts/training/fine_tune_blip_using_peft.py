...
def create_finetune_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Fine-tune BLIP model using PEFT on external datasets")
    ...
    parser.add_argument(
        "--max_new_tokens", type=int, default=64, help="Maximum new tokens for generation during sampling"
    )
...
def main():
    ...
    peft_config = PeftConfig(
        rank=args.rank, alpha=args.alpha, rslora=False, target_modules=args.target_modules, init_lora_weights="gaussian"
    )
